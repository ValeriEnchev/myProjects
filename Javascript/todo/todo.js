console.log("To-do - Pirpil: ES6 - Project 1");


document.title = "To-Do App";
document.getElementById("msgError").innerHTML = "This is an error message.";
...
function showBlockId(id) {
	...
	let useDiv = document.getElementById(id);
...
function showBlockSel(sel) {
	...
	let useDiv = document.querySelectorAll(sel);
...
function readLsTodoData() {
    ...
    let lsData = localStorage.getItem('todoAppData');
	if (lsData === null) {
		lsData = [];
	}
	return lsData;
...
*/
	let itemObj = {item: "call Jane", done: "no"};
	let itemsObj = [{item: "go work", done: "yes"}, {item: "pay bills", done: "no"}, {item: "shoping", done: "no"}];
	let listObj = {listName: "today", items: itemsObj};
	let listsObj = [listObj, {listName: "tomorrow", items: [itemObj]}];
	let userObj = {firstName: "John", lastName: "Doe", email: "jdoe@example.com", pwd: "1443235245985799"};
	let todoAppObj = [{user: userObj, lists: listsObj}];
	...
	let todoAppJSON = JSON.stringify(todoAppObj);
    document.getElementById("todoData").innerText = todoAppJSON; 
	
	let localStorageData = localStorage.getItem('todoAppData');
...
function getLoginInfo(loginInfo, loginPwd) {
	...
	//loginInfo = 'jdoe@example.com';
	console.log("loginInfo: " + loginInfo + " and loginPassword: " + loginPassword); 
	let loginUserInfo = getLoginUserInfo(userDataObj, loginInfo);
...
function updateLoggedUser(userObj) {
	...
	let userEmail = loggedUserInfo.email; 
	let userIdx = getIdx(loggedUserInfo.email, 'user.email', userDataObj);
    ...
    if ( tmpErrMsg.length > 0 ) {
		...
		if ( newUserObj !== null ) {
			if ( loggedUserInfo === null ) {
				console.log("This is Sing Up user!");
	
				console.log(isUniqueValue( newUserObj.email, 'user.email', userDataObj ));
				if ( isUniqueValue( newUserObj.email, 'user.email', userDataObj ) ) {
					//console.log(newUserObj);
					...
					showErrMessage( newUserObj.email + " is already exists!");					
				}
				...
				if ( secretWord !== '' ) {
					newUserObj.pwd = secretWord;
				console.log("The password is "+newUserObj.pwd);
				}
...
function loginFormSubmit() {
	...
	if (emailElem.value === "") {
		...
		let userInfo = getLoginInfo(emailElem.value, pswElem.value);
		if (userInfo !== null) {
...
const buttonCreateTodoList = document.getElementById("create-todo-list");
buttonCreateTodoList.addEventListener("click", () => {
...
const buttonAccount = document.getElementById("account-settigns");
...
const buttonAddListName = document.getElementById("saveListName");
buttonAddListName.addEventListener("click", saveListName);

function saveListName() {

	console.log("saveListName");

	let listName = document.getElementById("inputNewListName").value;
	if ( listName !== '' ) {
		...
		//console.log(dataObj[u].user.email);
			if ( dataObj[u].user.email === loggedUserEmail) {	
				...
				for (let l in loggedUserLists) {
					...
					let listInputCheckBox = document.createElement("input"); 
					...
					for (let i in loggedUserLists[l].items) {
						...
						let sublistLabel = document.createElement("label");
						...
						let sublistInput = document.createElement("input");
						...
						//let checkboxTaskDone = document.getElementById("s-l0-t0");
					...
					let buttonSaveNewTask = document.createElement("input");
					...
					let buttonShowNewTaskForm = document.createElement("input");
...
//	console.log(e.target.parentElement.children[0].id); 	// id="myLabel1"	
    ...
    let myLabelId = e.target.parentElement.children[0].id;
	let myLabelElement = document.getElementById(myLabelId);
	...
	let displayedListName = myLabelElement.innerText;
	let storedListNameElement = loggedUserLists[getIdx(displayedListName, 'name', displayedListNames)];
...
//	target.nextSibling.parentElement.previousSibling.previousSibling
	...
	let prefixID = e.target.parentElement.parentElement.id; // id="sub-list-l0"	
	let lst = e.target.parentElement.parentElement.firstElementChild.children.length;
	let tmpText = e.target.previousElementSibling.value;
	...
	//listLabel.id = "h-label-" + lst;
		...
		let checkboxComplited = document.createElement("input");
...
//	console.log(displayedListNames);
	...
	//let displayedListName = e.target.parentElement.parentElement.previousElementSibling.children[0].textContent; //today
	//let displayedListName = e.target.parentElement.parentElement.previousElementSibling.firstElementChild.innerText;	//today
	let displayedListName = e.target.parentElement.parentElement.parentElement.previousElementSibling.firstElementChild.innerText; //today
	let storedListNameElement = loggedUserLists[getIdx(displayedListName, 'name', displayedListNames)];
	// getIdx('today', 'name', displayedListNames)
	console.log("storedListNameElement: ");
	console.log(storedListNameElement.items);
...
//	let storedListItems = storedListNameElement.items;
	...
	let displayedTaskName = e.target.previousElementSibling.innerText;
...
//	let storedTaskNameElement = storedListItems[getIdx(displayedTaskName, 'name', displayedTaskNames)];
	let storedTaskNameElement = storedListNameElement.items[getIdx(displayedTaskName, 'name', displayedTaskNames)];
